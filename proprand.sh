...
##Switch the accession numbers to genotype
...
#Simplify input files and put the files on one line, shuffles them
#Chose the right number using the translated *per_geno input file
#NS5B selection is _2688_2727
#NS2 selection is 905_944

rm -rf *oneline* *NS5B*random*
...
rename 's/2688_2727_ps_with_duplicates\.fna\.one\.tab\.pasted\.goodfna\.oneline/NS5B_oneline\.fna/g' *oneline
rename 's/905_944_ps_with_duplicates\.fna\.one\.tab\.pasted\.goodfna\.oneline/NS2_oneline\.fna/g' *oneline
...
#Example names below
#P13_310310_1a_NS5B_oneline.fna
#P13_310310.pergeno
...
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '1q;d' $rand |cut -f 1)_NS5B_oneline.fna" > $rand\_ipa;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '2q;d' $rand |cut -f 1)_NS5B_oneline.fna" > $rand\_ipb;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '3q;d' $rand |cut -f 1)_NS5B_oneline.fna" > $rand\_ipc;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '4q;d' $rand |cut -f 1)_NS5B_oneline.fna" > $rand\_ipd;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '5q;d' $rand |cut -f 1)_NS5B_oneline.fna" > $rand\_ipe;done
...
for allns5b in *.pergeno; do cat "$(ls $allns5b|sed 's/\.pergeno/_/g')"*.NS5Brandomised > $allns5b\.NS5Brandomised\.all;done
rename 's/\.pergeno//g' *NS5Brandomised.all

rm -rf *_num_* *run1 *run2 *run3 *run4 *run5 *NS5Brandomised *oneline* *.total *pergeno*

##for rand in *905*.oneline; do cat $rand |shuf |head -n 5 |awk '{printf("%s\n%s\n",$1,$2)}'>$rand\.NS2randomised; done
