...
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '1q;d' $rand |cut -f 1)_NS2_oneline.fna" > $rand\_ipa;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '2q;d' $rand |cut -f 1)_NS2_oneline.fna" > $rand\_ipb;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '3q;d' $rand |cut -f 1)_NS2_oneline.fna" > $rand\_ipc;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '4q;d' $rand |cut -f 1)_NS2_oneline.fna" > $rand\_ipd;done
for rand in *.pergeno; do echo "$(ls $rand|sed 's/\.pergeno/_/g')$(sed '5q;d' $rand |cut -f 1)_NS2_oneline.fna" > $rand\_ipe;done
...
for allns2 in *.pergeno; do cat "$(ls $allns2|sed 's/\.pergeno/_/g')"*.NS2randomised > $allns2\.NS2randomised\.all;done
rename 's/\.pergeno//g' *NS2randomised.all

rm -rf *_num_* *run1 *run2 *run3 *run4 *run5 *NS2randomised *oneline* *.total *pergeno*
